		...
		for (corner = 0; corner < kMaxCorners; corner++)
		{
            ...
            for (i=0; i < 2; i++) {
                if (weight[row] == i) {
                    ...
                    tempmat4 = Mult(g_bones[i].rot,T(-g_bones[i].pos.x,-g_bones[i].pos.y,-g_bones[i].pos.z));

                    tempvec3 = MultVec3(tempmat4,g_vertsOrg[row][corner]);

                    g_vertsRes[row][corner] = MultVec3(T(g_bones[i].pos.x,g_bones[i].pos.y,g_bones[i].pos.z),tempvec3);
                }
            }
            // if (weight[i])
            //      take bone 1
