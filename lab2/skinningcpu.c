...
void DeformCylinder()
{
    ...
    mat4 tempmat4;
    vec3 tempvec3;
    vec3 cont_bone0, cont_bone1; //contribution from them bonez
	...
	for (row = 0; row < kMaxRow; row++)
	{
		for (corner = 0; corner < kMaxCorners; corner++)
		{
                    ...
                    tempmat4 = Mult(g_bones[0].rot,T(-g_bones[0].pos.x,-g_bones[0].pos.y,-g_bones[0].pos.z));

                    tempvec3 = MultVec3(tempmat4,g_vertsOrg[row][corner]);

                    cont_bone0 = ScalarMult(
                            MultVec3(T(g_bones[0].pos.x,g_bones[0].pos.y,g_bones[0].pos.z),tempvec3),
                            1-weight[row]);



                    tempmat4 = Mult(g_bones[1].rot,T(-g_bones[1].pos.x,-g_bones[1].pos.y,-g_bones[1].pos.z));

                    tempvec3 = MultVec3(tempmat4,g_vertsOrg[row][corner]);

                    cont_bone1 = ScalarMult(
                            MultVec3(T(g_bones[1].pos.x,g_bones[1].pos.y,g_bones[1].pos.z),tempvec3),
                            weight[row]);

                    g_vertsRes[row][corner] = VectorAdd(cont_bone0,cont_bone1);
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Them CPU bones");
